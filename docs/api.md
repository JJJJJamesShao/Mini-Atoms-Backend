...
1. **内容审核**：命中敏感词 → `400 CONTENT_BLOCKED`，响应体含 `category` 字段（`pornographic`｜`violence`｜`illegal`｜`hate`｜`self_harm`），前端可据此差异化提示；第一层关键词覆盖中文/英文与常见混淆变形（`p0rn`、拆字等）。第二层 LLM 语义过滤默认关闭（`LLM_FILTER_ENABLED=true` 开启），仅第一层通过后触发，拦截时响应体同构（`detail` 为审核理由）
