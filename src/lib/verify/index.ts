...
function posToLineCol(html: string, pos: number): { line: number; column: number } {
...
export function validateSyntax(html: string): VerifyError[] {
  ...
  scripts.forEach(({ index, code, startPos }) => {
    try {
      ...
      const globalColumn =
        err.loc?.line === 1 ? scriptStart.column + (err.loc?.column ?? 0) : (err.loc?.column ?? 0);
...
export function validateSecurity(html: string): VerifyError[] {
  ...
  root.querySelectorAll("a[href^='javascript:'], a[href^='JAVASCRIPT:']").forEach((el) => {
    const outer = el.outerHTML;
    const pos = html.indexOf(outer);
    const { line, column } = posToLineCol(html, pos);
  ...
  root.querySelectorAll('form[action]').forEach((el) => {
    ...
    errors.push({
      ...
      column,
      snippet: extractSnippet(html, pos, 1),
      suggestion: '移除 form 的 action 属性，所有交互应在页面内用 JavaScript 处理',
    });
  });
