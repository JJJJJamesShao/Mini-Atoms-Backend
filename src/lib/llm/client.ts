...
export async function streamGLM(
  ...
  console.log('[LLM] streamGLM 路由:', {
    model,
    provider: 'GLM',
    host: hostOf(process.env.GLM_BASE_URL ?? 'https://open.bigmodel.cn/api/paas/v4'),
  ...
  return client.chat.completions.create(body as OpenAI.Chat.ChatCompletionCreateParamsStreaming, {
    signal: options?.signal,
  });
}
