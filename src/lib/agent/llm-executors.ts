...
import type { ClarifyOutput, GenerateOutput, LocateOutput, SpecOutput } from '../schemas';
import type OpenAI from 'openai';
import { streamChat, streamGLM } from '../llm/client';
import { collectStreamText, StreamTruncatedError, throttleByChars } from '../llm/stream';
...
import { mergeToSingleHtml, parseCodeArtifact, wrapHtmlAsArtifact } from '../schemas/code-artifact';
...
export class CodeSummarizer {
  ...
  constructor(
    private readonly summarizeFn: (snippet: string) => Promise<string> = defaultSummarize,
  ) {}

  /** 满足节流条件时异步生成摘要并回调；否则立即返回 */
  async maybeSummarize(content: string, onSummary: (summary: string) => void): Promise<void> {
    const now = Date.now();
...
export function createLLMExecutors(
  ...
  return {
    ...
    spec: async (clarify, feedback) => {
      ...
      console.log('[DEBUG] Spec 响应: requirements =', result.requirements?.length ?? 0);
      memory.spec.add({
    ...
    generate: async (spec, errors, currentFiles, attempt = 0, stage) => {
      ...
      try {
        ...
        if (stage) {
          const stageFile =
            stage === 'schema' ? 'schema.js' : stage === 'shell' ? 'shell.html' : 'pages.js';
          bus?.emit({
            type: 'agent:progress',
            agent: agentName,
            role: '前端工程师',
        ...
        if (options?.structured) {
          ...
          bus?.emit({
            ...
            role: '前端工程师',
            percent: 10,
            message: isFixMode ? 'Patch 失败，回退到完整重写...' : '正在调用 GLM-5.2 生成代码...',
          });
    ...
    verify: async (files, stage) => {
      ...
      const result = stage ? verifyStageCode(files[0]?.content ?? '', stage) : verifyProject(files);
      memory.verify.add({
        topic: MessageTopic.REVIEW,
        content: JSON.stringify(result),
        metadata: { direction: 'out' },
