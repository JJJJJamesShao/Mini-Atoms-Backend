...
/** 将文本切分为行并记录每行的起始字符偏移 */
function splitWithOffsets(text: string): Array<{ line: string; start: number }> {
...
const MATCH_STRATEGIES: Array<{
  ...
  {
    level: 'rstrip',
    find: (content, search) => findLineMatches(content, search, (a, b) => rstrip(a) === rstrip(b)),
  },
  {
    level: 'indent',
    find: (content, search) =>
...
export function formatPatchFeedback(result: ApplyResult): string {
  ...
  const lines: string[] = [];
  lines.push(`⚠️ Patch 应用结果：${result.applied} 成功，${result.failed} 失败`);
