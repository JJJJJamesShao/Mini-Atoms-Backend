...
const FULLSTACK_PATTERN = /博客|管理系统|数据库|登录|注册|后台|crud|增删改查|blog|admin|dashboard/i;
...
export function selectSOPId(input: string, opts?: { hasCurrentCode?: boolean }): string {
...
/** 选择 SOP 配置（保证注册表中存在） */
export function selectSOP(input: string, opts?: { hasCurrentCode?: boolean }): SOPConfig {
