...
import { applyPatch, formatPatchFeedback, parsePatch, summarizeAppliedPatch } from './patch';
...
function stageGenerateOutput(ctx: ExecutionContext, key: string): GenerateOutput | undefined {
  const out = ctx.stageOutputs[key];
  return out && 'files' in out ? out : undefined;
}

/** 多阶段 SOP：当前阶段生成时作为输入的前置产物 */
function stageInputFiles(ctx: ExecutionContext, stage: string | undefined): File[] | undefined {
  switch (stage) {
    case 'schema':
  ...
  while (current) {
    ...
    if (step.action === 'done') {
      return {
        ...
        stageOutputs: Object.keys(ctx.stageOutputs).length > 0 ? ctx.stageOutputs : undefined,
      };
    }
    if (step.action === 'fail') {
  ...
  switch (step.action) {
    ...
    case 'verify': {
      ...
      ctx.patchFeedback = out.pass
        ? undefined
        : out.errors.map((e, i) => `[错误 ${i + 1}] ${e.rule}：${e.message}`).join('\n');
      return out;
    }
    case 'locate': {
    ...
    case 'apply': {
      ...
      const summary = summarizeAppliedPatch(blocks, applied.details, ctx.locate?.anchors);
    ...
    case 'fix': {
      ...
      bus?.emit({
        type: 'agent:thinking',
        agent: step.name === 'fix-patch' ? 'patch' : step.name.replace(/^fix($|-)/, 'generate$1'),
