...
export function splitPages(pagesText: string): Map<string, string> {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const end =
      i + 1 < matches.length ? (matches[i + 1].index ?? pagesText.length) : pagesText.length;
...
export function findMissingPages(shellHtml: string, pagesText: string): string[] {
 ...
 */
export function mergeFullstack(schemaJs: string, shellHtml: string, pagesText: string): string {
  const pages = splitPages(pagesText);
