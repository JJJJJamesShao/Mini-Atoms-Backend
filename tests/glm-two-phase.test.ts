...
import { streamChat, streamGLM } from '../src/lib/llm/client';
import { createLLMExecutors } from '../src/lib/agent/llm-executors';
import { StreamTruncatedError } from '../src/lib/llm/stream';
import { GENERATE_MAX_TOKENS, parseEstimatedTokens, PLAN_MAX_TOKENS } from '../src/lib/llm/planner';
...
const PLAN_TEXT = '实现方案：单页面三区块布局，状态用数组管理。\n估算token：50000';
...
describe('两阶段生成', () => {
  it('阶段 1 思考开/32K，阶段 2 思考关/100K，方案注入出码消息', async () => {
    streamGLMMock
      .mockImplementationOnce(() => fakeStream([{ content: PLAN_TEXT, finish: 'stop' }]))
    ...
    expect(phase2Messages.at(-1)?.content).toContain('实现方案：单页面三区块布局');
