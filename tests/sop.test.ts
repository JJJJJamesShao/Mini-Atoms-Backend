...
describe('selectSOP 路由', () => {
  ...
  it('有现有代码（对话迭代）→ modify 增量修改小循环，优先级高于关键词', () => {
    expect(selectSOPId('把标题改成蓝色', { hasCurrentCode: true })).toBe('modify');
    expect(selectSOPId('做一个数独游戏', { hasCurrentCode: true })).toBe('modify');
    expect(selectSOPId('做一个数独游戏', { hasCurrentCode: false })).toBe('game');
    expect(selectSOP('把标题改成蓝色', { hasCurrentCode: true })).toBe(MODIFY_SOP);
  });
...
describe('runSOP 执行引擎', () => {
  ...
  it('fullstack-app：阶段校验失败只重修该阶段（lastErrors 不跨阶段污染）', async () => {
    ...
    const executors: Executors = {
      ...
      verify: async () => (verifyQueue.length > 1 ? verifyQueue.shift()! : verifyQueue[0]),
      locate: async (input) => ({ intent: input, anchors: [] }),
      patch: async (locate) => ({ patchText: '', notes: locate.intent }),
    };
    const out = await runSOP('做一个带登录的博客', FULLSTACK_SOP, executors, async () => true);
...
describe('modify SOP 增量修改小循环', () => {
  ...
  const LOCATE: LocateOutput = {
    intent: '把标题改成新标题',
    anchors: [{ id: 'a1', description: '主标题', searchHint: '<h1>旧标题</h1>' }],
  };
  const GOOD_PATCH = [
    '<<<<<<< SEARCH',
    '<h1>旧标题</h1>',
  ...
  function makeModifyExecutors(overrides?: {
    ...
    const executors: Executors = {
      ...
      patch: async (_locate, _files, feedback) => {
        ...
        const patchText = patchQueue.length > 1 ? patchQueue.shift()! : patchQueue[0];
