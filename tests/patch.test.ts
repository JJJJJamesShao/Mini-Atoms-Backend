...
import { applyPatch, parsePatch, summarizeAppliedPatch } from '../src/lib/agent/patch';
...
describe('applyPatch 三级匹配', () => {
  ...
  it('大小写敏感：大小写不同任何级别都不应命中', () => {
    const result = applyPatch(DOC, [{ search: '    LET COUNT = 0;', replace: 'x' }]);
...
describe('summarizeAppliedPatch 改动摘要', () => {
  ...
  it('块按 searchHint 映射回锚点，聚合字符数', () => {
    ...
    expect(summary).toBe('修改 3 处：主题色 CSS 变量（~26 字符）、按钮点击事件（~64 字符）');
  });

  it('未匹配锚点的块归入「其他改动」', () => {
    const blocks = [{ search: '<footer>old</footer>', replace: 'x' }];
    const details = [{ index: 0, status: 'applied' as const }];
    expect(summarizeAppliedPatch(blocks, details, ANCHORS)).toBe('修改 1 处：其他改动（~20 字符）');
  });

  it('无锚点（非 modify 流程）时全部归入「其他改动」', () => {
    const blocks = [{ search: 'a', replace: 'b' }];
    const details = [{ index: 0, status: 'applied' as const }];
    expect(summarizeAppliedPatch(blocks, details)).toBe('修改 1 处：其他改动（~1 字符）');
  });
