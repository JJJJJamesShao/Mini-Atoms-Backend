...
import type { ClarifyOutput, LocateOutput, SpecOutput } from '../src/lib/schemas';
...
describe('prompt 输出契约', () => {
  ...
  it('SYSTEM_GENERATE 必须包含 4 个 SECTION 分段标记（流式子步骤进度解析依赖）', () => {
    ...
    for (const marker of ['SECTION: HEAD', 'SECTION: CSS', 'SECTION: BODY', 'SECTION: JS']) {
...
describe('modify SOP prompt 输出契约', () => {
  ...
  it('buildLocatePrompt user 消息必须携带现有代码与修改需求', () => {
    const messages = buildLocatePrompt('<body>旧代码</body>', '把主题色改成暗色');
