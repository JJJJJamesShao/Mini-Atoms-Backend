...
describe('verifyStageCode', () => {
  it('schema：合法 JS 通过', () => {
    const r = verifyStageCode('const db = {};\nfunction list() { return []; }', 'schema');
  ...
  it('shell：缺占位符 / 缺 DOCTYPE / 外部脚本分别被拒绝', () => {
    expect(
      verifyStageCode('<!DOCTYPE html><html><head></head><body>无占位符</body></html>', 'shell')
        .pass,
    ).toBe(false);
    expect(
      verifyStageCode('<html><body><!-- PAGE_CONTENT:home --></body></html>', 'shell').errors.some(
        (e) => e.message.includes('DOCTYPE'),
      ),
    ).toBe(true);
    expect(
      verifyStageCode(
  ...
  it('pages：合法 JS 块通过，HTML 文档结构被拒绝', () => {
    expect(verifyStageCode('// === PAGE: home ===\nconst x = 1;', 'pages').pass).toBe(true);
    expect(verifyStageCode('<html><body>越权输出</body></html>', 'pages').pass).toBe(false);
  });
