...
import { collectStreamText, StreamTimeoutError, throttleByChars } from '../src/lib/llm/stream';
...
describe('collectStreamText 超时保护', () => {
  ...
  it('idle 超时：chunk 间隔超过上限抛 StreamTimeoutError(idle)', async () => {
    await expect(
      collectStreamText(fakeStream([{ text: '第一段' }, { text: '第二段', delayMs: 200 }]), {
        idleTimeoutMs: 50,
        totalTimeoutMs: 5000,
      }),
  ...
  it('超时时主动断开底层连接（controller.abort 被调用）', async () => {
    ...
    await expect(
      collectStreamText(fakeStream([{ text: 'x' }, { text: 'y', delayMs: 200 }], controller), {
        idleTimeoutMs: 50,
        totalTimeoutMs: 5000,
      }),
